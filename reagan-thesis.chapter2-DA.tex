...
For a selection of these algorithms, I illustrate some of their properties via a MATLAB implementation using the Lorenz 1963 three-variable model: %of the Extended Kalman Filter (EKF), Ensemble Kalman Filter (EnKF), and 3D-Var on the Lorenz 1963 three-variable model:
\begin{align*}
...
I begin by deriving the equations for optimal interpolation (OI) and for the Kalman Filter with one scalar observation.
The goal of my derivation is to highlight which statistical information is useful, which is necessary, and what assumptions are generally made in the formulation of more complex algorithms.
...
This was originally done by Aerology et al. (1960) %\shortcite{eliassen1954}
, and independently by Gandin (1965). %\shortcite{gandin1965}
...
Denote the observations as $\mby_0$, where the ordering is the same as $\mbx$ but the dimension is much smaller (e.g. by a factor of $10^3$ for NWP) % $\left | \mby _0 \right | << \left | \mbx \right |$ 
...
Examples of indirect and spatially heterogeneous observations in NWP are radar reflectivities and Doppler shifts, satellite radiances, and global positioning system atmospheric refractivities \shortcite{kalnay2003}.
...
Since there are typically less observations than there are model variables, 3D-Var is a more efficient method.
...
\begin{equation} \frac{d\mby }{dt } = \mathbf{J} \mby \ref{eq:ODETLM} \end{equation}
...
The problem with Equation \ref{eq:ODETLM} solving system of equations is that the Jacobian matrix of discretized code is not necessarily identical to the discretization of the Jacobian operator for the analytic system.
...
But, that is extremely messy (and costly), since Runge-Kutta methods have nested function iterations.
...
In order to verify my implementation of the TLM, I propagate a small error in the Lorenz 63 system and plot the difference between that error and the TLM predicted error in each variable forward in time in Figure \ref{fig:TLMverification}.
...
The spread of these models with random errors is then used to estimate the error covariance.
...
Computing the error covariance matrix for each forecast, that forecast itself is excluded from the ensemble average forecast.
...
\subsection{Performance of EnKF, EKF on Lorenz 63}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/DA_test_noname2.pdf}
  \caption{
    ...
    Initial error for DA is normal with mean 0 and variance 0.01, and the RMS errors reported are averages of 100 runs.
    Observational noise is normally distributed with mean 0 and variance 0.5 (approximately 5\% of climatological variation), and an assimilation window length of 0.261 is used.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/window_experiment_plot001.pdf}
  \caption{
...
The result of searching over these parameters for the optimal values to minimize the analysis RMS error differs for each window length, so here here we present the result for a window of length 210 seconds using the EKF.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/EKF_210s_infl_error.pdf}
  \caption{
...
With the formulation of the LEKF and the ETKF there is not much new to be said here, but rather a synthesis of the advantages of both of these approaches.
This is the method that was sufficiently efficient for implementation on the full OpenFOAM CFD model of 216,000 model variables, and so I present it in more detail and follow the notation of Hunt et al (2007). % \shortcite{hunt2007efficient}.
