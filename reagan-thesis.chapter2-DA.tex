...
The computational cost of the EKF is mitigated through the approximation of the error covariance matrix $\mbP_f$ from an ensemble of predictions.
...
In the prior, the random error added to the observation can be truly random \shortcite{harris2011predicting} or be drawn from a distribution related to the analysis covariance, which I chose.
Distributing ensemble members around the analysis forecast, with initial conditions perturbed as (for example) the square root of the ensemble has advantages compared to perturbed observations \shortcite{evensen2003ensemble}.
...
The only difference between this approach and the EKF, numerically, is that the forecast error covariance $\mbP^f$ is computed from the ensemble members, without the need for a tangent linear model:
...
This is another type of square root filter.
...
$$ \mathbf{A}_k = (1- \mbK_k \mbH ) \mbB _k .$$

The analysis covariance is written

$$ \mathbf{A} = \frac{1}{P-1}\mathbf{X}^a (\mathbf{X}^a) ^T = \mathbf{X} ^b \hat{\mathbf{A}} (\mathbf{X} ^b) ^T $$
...
See \shortcite{kalnay20074} for more detail.

\subsection{Local Ensemble Transform Kalman Filter (LETKF)}

The LETKF implements a strategy that becomes very important for large simulations.
...
We overcome this difficulty by placing a more stringent requirement on the mesh of localization of indexing, and extract the adjacecy matrix for use in filtering.
