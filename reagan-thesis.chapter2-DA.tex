...
For a selection of these algorithms, we illustrate some of their properties via a MATLAB implementation using the Lorenz 1963 three-variable model: %of the Extended Kalman Filter (EKF), Ensemble Kalman Filter (EnKF), and 3D-Var on the Lorenz 1963 three-variable model:
\begin{align*}
...
From the simpler examples, we build the most advanced 4D-Var and 4DLETKF approaches with the full CFD model (216K variables).
...
The PSAS data assimilation is so similar to 3D-Var that we do not derive it here, the only difference being that the cost function $J$ is solved in observation space.
Since there are typically less observations than there are model variables, 3D-Var is a more efficient method.

Finally we note for 3D-Var that operationally $\mbB$ can be estimated with some flow dependence with the NMC or NCEP method, i.e.
...
\[ \frac{d\mbx}{dt} = F(\mbx) .\]
We integrate this system using a numerical scheme of our choice (here we used RK2), to obtain a model $M$ discretized in time.
\[ \mbx(t) = M[ \mbx(t_0) ] .\]
...
In order to verify our implementation of the TLM, we propagate a small error in the Lorenz 63 system and plot the difference between that error and the TLM predicted error in each variable forward in time in Figure \ref{fig:TLMverification}.
...
In the prior, the random error added to the observation can be truly random \shortcite{harris2011predicting} or be drawn from a distribution related to the analysis covariance, which we chose for this type of filter.
...
Next, we present the results of a MATLAB implementation of the EKF and basic EnKF.

\subsection{Performance of EnKF, EKF on Lorenz 63}

With the EnKF and the EKF algorithms coded as above, we present some basic results on their performance. 
...
This is the method that was sufficiently efficient for implementation on the full OpenFOAM CFD model of 216,000 model variables, and so we present it in more detail and follow the notation of Hunt et al (2007). % \shortcite{hunt2007efficient}.
