...
The general formulation of the LEKF by Ott \shortcite{ott2004local} goes as follows.
First, determine the 
...
This is the method that was efficient enough for implementation on the full OpenFOAM CFD model, and so I present it in more detail.
...
Main issues with the 4D-Var approach include attempting to parallelize what is otherwise a sequential algorithm, see work done by \shortcite{fisher}.


\subsection{Ensemble-variational hybrids}

In ensemble-variational hybrid approaches, benefits of flow dependent covariance estimates obtained from an ensemble method and the computational efficiency of interatively solving a cost function to obtain the analysis are combined.
An ensemble of forecasts is used to update the static error covariance $B$ in the formulation of 4D-Var (this could be done in 3D-Var as well) and one possible reformulation is to rewrite the cost function with error covariance $P^f$.
\begin{equation} \label{eq:En4DVarCost} J(\delta x) = ... \end{equation}

Combining the static error covariance $B$ and the flow dependent error covariance $P^f$ is an active area of research, cite someone: \shortcite{someone}.
