...
The goal of this chapter is to introduce and detail the practical implementation of state of the art data assimilation algoritms.
I derive the general form for the problem, and then talk about how each filter works.
For a selection of these algorithms, I illustrate some of their properties via a MATLAB implentation on the Lorenz 1963 three-variable model: %of the Extended Kalman Filter (EKF), Ensemble Kalman Filter (EnKF), and 3D-Var on the Lorenz 1963 three-variable model:
\begin{align*}
...
\begin{figure}[h!]
  ...
  \caption{
    ...
    In other words, larger numbers represent where forecasts are poor, and the data assimilation algorithm puts more weight on the observation.
  }
...
I begin by deriving the equations for OI and for the Kalman Filter with one scalar observation.
The goal of my derivation is to highlight which statistical information is useful, which is necessary, and what assumptions are generally made in for formulation of more complex algorithms.

Beginning the one variable $U$, which could represent the horizontal velocity at one gridpooint, say we make two independent observations of $U$: $U_1$ and $U_2$.
If the true state of $U$ is given by $U_t$, then the goal is to produce an anaylsis of $U_{1,2}$ which we call $U_a$, that is our best guess of $U_t$.

Denote the errors of $U_{1,2}$ by $\epsilon _{1,2}$ respectively, such that
$$ T_{1,2} = T_t + \epsilon _{1,2} .$$
...
This is written as
$$ E(T_{1,2}-T_t) = E(\epsilon _{1,2} ) = 0.$$
...
This is unrealistic, and to move forward we assume that we know the variance of each $T_{1,2}$.
With this knowlodge, which we write
...
We again attempt a linear fit of these observations to the truth, and can do better than an average.
For $U_a$ our best guess of $U_t$ we write this linear fit as
\begin{equation} U_a = \alpha _1 T_1 + \alpha _2 T_2 \label{eq:analproblem}. \end{equation}
If $U_a$ is to be our best guess, we desire it to unbiased
$$E(U_a) = E(U_t) \Rightarrow \alpha _1 + \alpha _2 = 1$$
and to have minimal variance error $\sigma _a ^2$.
...
Of these, knowing the error variances (and in multiple dimensions, error covariance matrices) will become our focus and better solutions to determining this have been largely responsible for major improvements in forecast skill.

Solving \ref{eq:analproblem} by substituting $\alpha _2 = 1 - \alpha _1$ have
...
To minimize $\sigma _a^2$ with respect to $\alpha _1$ we compute $\frac{\partial }{\partial \alpha _1}$ of the above and solve for 0 to obtain that
...
This last formula says that given accurate statistical knowledge of $U_{1,2}$, the precision of the analysis is the sum of the precisions of the measurements.

We can now formulate the simplest sequence data assimilation cycle by using our forecast model to generate a background forecast $U_b$ and setting $U_1 = U_b$.
We then assimilate the observations $U_o$ by setting $U_2 = U_o$ and solving for the analysis velocity $U_a$ as
\begin{equation} U_a = U_b + W (T_o - T_b) .\end{equation}

The difference $T_o - T_b$ is commonly reffered to as the ``innovation'' and represents the new knowledge from the observations with respect to the backgroud forecast.
The optimal weight $W$ is given by \ref{eq:analprec} then as
\begin{equation} W = \frac{\sigma _b ^2}{\sigma_b ^2 - \sigma _o ^2} = \sigma_b ^2 (\sigma_b^2 - \sigma_o^2) ^{-1} .\end{equation}

The analysis error variance and precision are defined as before, and for convience we can rewrite this in terms of $W$ as \shortcite{kalnay2003}:
\begin{equation} \sigma _a ^2 = (1-W ) \sigma_b^2 .\end{equation}

We now generalize the above for models of more than one variable to obtain the full equations for OI.
This was originally done by \shortcite{eliassen1954}, and independently by \shortcite{gandin1965}.
...
We combine all the variables into one list $\mbx$ of length $n$ where $n$ is the product of the number of grid points of the number of model variables.
...
Denote the observations as $\mby_0$, where the ordering is the same as $\mbx$ but the dimension is $\left | \mby _0 \right | \leq \left | \mbx \right |$ and the observations are not strictly at the grid points of $\mbx$ or even direct measurements of the variables in $\mbx$.
In our experiment, temperature measurements are in the model variable $T$, but are understood as the average of the temperature of the grid point surrounding it within some chosen radius.
Examples of indirect and spatially heterogenous observations in model weather forecasting are radar reflectivites and Doppler shits, satellite radiances, and global positioning system (GPS) atmospheric refractivities \shortcite{kalnay2003}.



We can now write the write that analysis with similar notation as for the scalar case (no additional explanation is necessary) with the addition of the observation operator $H$ which transforms model variables into observation space:

\begin{align} \mbx_a &= \mbx_b + \mbW \mbd \\
\mbd &=  \mby_0 - H (\mbx _b ) \\
\mbe _a  &= \mbx_t - \mbx_a . \end{align}

Note that it is advatageous to write $H$ as an $n \times n_{\text{obs}}$ matrix, where $H$ is not required to be linear.
...
\begin{align*} \expv{ \mbe_b} &= \expv{ \mbx _b) - E(\mbx _t } = 0\\
...
Since the background is a decent forecast, it is convienent to write our equations in terms of changes to $\mbx _b$.
...
&= \left ( \cancel{E\left [ - \mbe _b \mbe _o ^T \right ]} + E \left [ \mbe _b \mbe _b ^T \right ] \mbH ^T \right ) \left ( E \left [ \mbe _o \mbe _o ^T \right ] - \cancel{\mbH E\left [ \mbe _b \mbe _o ^T \right ]} - \cancel{E \left [ \mbe _o \mbe _b ^T \right ] \mbH ^T } + \mbH E \left [ \mbe _b \mbe _b ^T \right ] \mbH ^T \right ) ^{-1}\\
...
&= \expv{\mbe_b\mbe_b^T + \mbe_b (\mbe _o^T -  \mbe _b^T \mbH ^T )\mbW ^T + \mbW (\mbe _o - \mbH \mbe _b )  \mbe _b ^T + \mbW (\mbe _o - \mbH \mbe _b )  (\mbe _O^T -  \mbe _b^T \mbH ^T )\mbW ^T}\\
&= \expv{\mbe_b\mbe_b^T}  + \expv{\mbe_b (\mbe _o^T -  \mbe _b^T \mbH ^T )} \mbW ^T + \mbW \expv{ (\mbe _o - \mbH \mbe _b )  \mbe _b ^T} + \mbW \expv{(\mbe _o - \mbH \mbe _b )  (\mbe _O^T -  \mbe _b^T \mbH ^T )}\mbW ^T\\
&= \mbB - \mbB \mbH ^T \mbW^T - \mbW \mbH \mbB + \mbW \expv{\mbe_o\mbe_o^T - \cancel{\mbe_o\mbe_b^T}\mbH ^T - \cancel{\mbH \mbe _b \mbe _o ^T } + \mbH \mbe_b \mbe_b^T \mbH ^T }\mbW ^T\\
...
Note that in this formulation, we have assumed that $\mbR$ (the observation error covariance) accounts for both the instrumental error and the ``representativeness'' error, a term coined by Lorenc to describe the fact that the model does not account for subgrid-scale variability \shortcite{lorenc1981global}.
...
As a final note, it is also possible to derive the ``precision'' of the analysis $\mathbf{P} _a ^{-1}$ but in practice it is impractical to compute the inverse of the full matrix $\mathbf{P}_a$, so I omit this derivation.
...
For different models, this equates to different assumptions on the correlations of observations, which I do not consider here.
...
The usefulness of the variational approach comes from the computational efficiency.
This is why the 3D-Var analysis scheme became the most widely used data assimilation technique in operational weather forecasting, only to be supplemented by 4D-Var today.

The formulation of the cost function can be done in a Bayesian framework \shortcite{purser1984new} or using maximum likelihood (ML) approach (below).
First, it is not difficult to see that solving for $\partial J/ \partial U = 0$ for $U = U_a$ where we define $J(U)$ as
\begin{equation} J(T) = \frac{1}{2} \left [ \frac{(U-U_1)^2}{\sigma_1^2}+\frac{(U-U_2)^2}{\sigma_2^2} \right ] \end{equation}
gives the same solution as obtained through least squares in our scalar assimilation.
The formulation of this cost function obtainined through the ML approach is simple \shortcite{edwards1984likelihood}.
...
The most likely value of $U$ given two observations $U_{1,2}$ is the value which maximizes the joint likelihood (the product of the likelihoods), hence maximizes the logarithm of the joint likelihood (since the logarithm is increasing):
...
Jumping straight to result of Lorenc \shortcite{lorenc1986analysis} (we could define likelihood functions just like above), we define the multivariate 3D-Var as finding the $\mbx _a$ that minimizes the cost function
\begin{equation} J(\mbx) = (\mbx - \mbx_b) ^T \mbB ^{-1} (\mbx - \mbx_b) + (\mby_o + H(\mbx))^T\mbR (\mby_o - H(\mbx)) .\end{equation}

The formal minimization of $J$ can be obtained by solving for $\nabla_x J(\mbx_a)$, but since this is not useful in practice (iterative methods are used), we only present the result from \shortcite{kalnay2003}:
\begin{equation} \mbx_a = \mbx_b+(\mbB^{-1} + \mbH^T\mbR^{-1}\mbH)^{-1}\mbH^T\mbR^{-1}(\mby_o-H(\mbx_b)) .\end{equation}

The PSAS data assimilation is so similar to 3D-Var that I do not derive it here, the only difference being that the cost function $J$ is solved in observation space.
Since there are typically less observations than there are model variables, this is a more efficient method.

Finally I note for 3D-Var that operationally $\mbB$ can be estimated with some flow dependence with the NMC or NCEP method, i.e.
...
The TLM is precisely the model (written as a matrix) that transforms a pertubation at time $t$ to a perturbation at time $t+\Delta t$, analytically equivalent to the Jacobian of the model.
...
Since we are making observations of the truth with random normal errors of standard deviation $\mbe$, then $\mbR$ is a matrix of just $\epsilon$ values.
...
\[ \mbx(t) = M[ \mbx(t_0) ] .\]
Introducing a small perturbation $\mby$, we can approximate the our model $M$ of $\mbx(t_0) + \mby(t_0)$ with a Taylor series around $\mbx(t_0)$, for which we assume to already have solved for:
...
\[ \frac{d\mby }{dt } = \mathbf{J} \mby \] where $\mathbf{J} = \partial F / \partial \mbx$ is the Jacobian of $F$.
...
We can apply our numerical method to the $d\mbx/dt = F(\mbx)$ to obtain $M$ explicitly, and then take the Jacobian of this.
...
In order to verify my implementation of the TLM, I propogate a small error in the Lorenz 63 system and plot the difference between that error and the TLM predicted error in each variable forward in time in Figure \ref{fig:TLMverification}.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.49\textwidth]{figures/TLM-verification003_noname.pdf}
  \caption{
...
One such approach is the use of a forecast ensemble, where a collection of models (ensemble members) are initialized from the prior analysis $\mbx_a$ with random error.
The spread of these models with randoms errors is then used to estimate the error covariance.
In the limit of infinite ensemble size, the EnKF is equivalent to the EKF \shortcite{evensen2003ensemble}.
There are two prevailing methodologies for maintaining independent ensemble members: adding errors to the observation used by each forecast and assimilating each ensemble individually or distributing ensemble initial conditions from the analysis prediction.
In the prior, the random error added to the observation can be truly random \shortcite{harris2011predicting} or be drawn from a distribution related to the analysis covariance, which I chose for this type of filter.
The latter method of distributing ensemble members around the analysis forecast, with initial conditions perturbed as (for example) the square root of the ensemble has advantages compared to perturbed observations \shortcite{evensen2003ensemble}.
...
The specific factors used are tuned through an exhaustive search, and included in the next section.
...
The forecast from an ensemble is the average of the ensemble forecasts, and an explanation for this choice is substantiated by Burgers \shortcite{burgers1998analysis}.
The general EnKF which I use is most similar to that of Burgers.
Many algorithms based on the EnKF have been proposed and include the Ensemble Transform Kalman Filter (ETKF) \shortcite{ott2004local}, Ensemble Analysis Filter (EAF) \shortcite{anderson2001new}, Ensemble Square Root Filter (EnSRF) \shortcite{tippett2003ensemble}, Local Ensemble Kalman Filter (LEKF) \shortcite{ott2004local}, and finally the Local Ensemble Transform Kalman Filter (LETKF) \shortcite{hunt2007efficient}.
I explore some of these in the following sections.
A comprehensive overview through 2003 is provided by Evensen \shortcite{evensen2003ensemble}.
Next, I present the results of a MATLAB implementation of the EKF and basic EnKF.

\subsection{Performace of EnKF, EKF on Lorenz 63}

With the EnKF and the EKF algorithms coded as above, I present some basic results on their performance. 
...
Next we test the performace of each filter against increasing assimilation window length.
As we would expect, longer windows make forecasting more challenging for both algorithms.
The ensemble filter begins to out-perform the extended kalman filter for longer window lengths because the ensemble more accuraately captures the nonlinearity of the model.
...
\begin{figure}[h!]
  ...
  \caption{
    ...
    Additive and multiplicative covariance inflation is the same as Harris et. al.
  }
  \label{fig:window_test}
\end{figure}

Both filters are sensitive to the size of the covariance matrix, and in some cases to maintain the numerical stability of the assimilation it is necessary to use additive and/or multiplicative covariance inflation.
In both the EKF and the EnKF mutliplicative error is performed with inflaction factor $\Delta$ by
\begin{equation} \mbK = (1+\Delta) \mbK .\end{equation}.
Additive inflation is performed for the EKF as in \shortcite{yang2006} with inflaction factor $\mu$, where uniform random numbers in $[0,\mu]$, here $\nu$ representing a vector of length $n$ of such numbers:
\begin{equation} \mbK = \mbK + \text{diag}(\nu) .\end{equation}
For the EnKF we perform additive covariance inflation to the analysis state itself for each ensemble:
\begin{equation} \mbx  = \mbx + \nu .\end{equation}
The result of search over these parameters for the optimal values to minimize the analysis RMS error difffers for each window length, so here here we present the result for a window of length 210 seconds on the EKF.
In Figure \ref{fig:EKF_cov_tuning_210s} we see that there is a clear region of parameter space for which the filter performs with lower RMS, although this region appears to extend into values of $\Delta > 3$.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.49\textwidth]{figures/EKF_210s_infl_error.pdf}
  \caption{
    The RMS error averaged over 100 model runs of length 1000 windows is reported for the EKF for varying additive and multiplicative inflation factors $\Delta$ and $\mu$.
  ...
  \label{fig:EKF_cov_tuning_210s}

\end{figure}

\subsection{Ensemble Transform Kalman Filter (ETKF)}

The ETKF introduced by Bishop is one type of square root filter, and I present it here to provide background for the formulation of the LETKF \shortcite{bishop2001adaptive}.
...
Because $\mbP_a,\mbP_f$ are symmetric positive-definite (by defintion) we can write
...
Per \shortcite{tippet2003ensemble}, the solution for $\mbZ_a$ can be solved by perturbed observations
...
From \shortcite{tippet2003ensemble} we note that the matrix $\mathbf{C}$ of orthonormal vectors is not unique.
...
In this filter, the analysis perturbations are assumed to be equal to the backgroud perturbations postmultiplied by a transformation matrix $\mathbf{T}$ so that that the analysis error covariance satisifies
...
To summarize, the steps for the ETKF are to (1) form $\mbZ_f^T\mbH^T\mbR^{-1}\mbH\mbZ_f$, assuming $\mathbf{R}^{-1}$ is easy. Then (2) compute the eigenvalue decomposition of this, and apply it $\mbZ_f$.


\subsection{Local Ensemble Kalman Filter (LEKF)}

The LEKF implements a strategy that becomes very important for large simulations: localization.
Namely, the analysis is computed for each gridpoint using only local observations, without the need to build matrices that represent the entire analysis space.
This localization removes long-distance correlations from $\mathbf{B}$ and allows greater flexibility in the global analysis by allowing different linear combintations of ensemble members at different spatial locations \shortcite{kalnay20074}.
...
In fact, they are decidely non-adjacent for certain meshes, with every other index corresponding to opposing sides of the loops.
We overcome this difficulty by placing a more stringent requirement on the mesh of localization of indexing, and extracting the adjacecy matrix for use in filtering.

The general formulation of the LEKF by Ott \shortcite{ott2004local} goes as follows, directly from his paper:
\begin{enumerate}
...
Figure \ref{fig:ott2004algorithm} depicts this cycle in greater detail, with similar notations to that used thus far (from \shortcite{ott2004local}).

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.49\textwidth]{figures/ott2004algorithmPic.png}
  \caption{
    The algorithm for the LEKF as described by Ott.
    Building on previous work, the algorithm is as one would expect.
  }
  \label{fig:ott2004algorithm}
\end{figure}

There is not much difference than from what one would build naturally to perform the EnKF with localization in Ott's work \shortcite{ott2004local}, with the exception of the reconstruction of the global analysis from local analyses.
Since each of the assimilations were performed locally, we cannot be sure that there is any smoothness, or physical balance, between adjacent points.
To ensure a unique solution of local analysis perturbations that has some respect of this balance, utilization of the prior background forecasts as reasonably accurate is used to constrain the construction of local analysis perturbations to minimize the difference from the background.
...
With the formulation of the LEKF and the ETKF there is not much new to be said here, but rather a systhesis of the advantages of both of these approaches.
This is the method that was sufficiently efficient for implementation on the full OpenFOAM CFD model of 216,000 model variables, and so I present it in more detail and follow the notation of \shortcite{hunt2007efficient}.
...
In addition to this simple scheme, Hunt proposes weighing observations by their proximity to smoothly transistion across them.
...
Finally, we linearize $H$ by applying it to each $\mbx_{b(i)}$ and inerpolating.
...
\item comptue $\overline{\mbw} _a  = \tilde{\mbP}_a \mathbf{C} \left ( \mby_o - \overline{\mby} _b \right )$ and add it to the column of $\mbW_a$
...
Extending the LETKF to incorporate observations from within the assimilation time window also become useful with high-frequency temperature sensors.
...
The next generation supercomputers that will be used in numerical weather prediction will allow for increased model resolution, but the proper scaling of this computation with the number of processors.
Main issues with the 4D-Var approach include attempting to parallelize what is otherwise a sequential algorithm and the computational difficulty of the strong-constraint formulation with many TLM integrations, see work done by \shortcite{fisher}.
...
At the ECMWF, their 4D-Var is run on a long-window cycle (24 and 48 hours) so that many observations are made within the time window.
Generally, there are two main 4D-Var formulations in use: strong-constraint and weak-constaint.
The strong-constraint formulation uses the forecast model as a strong constraint, and is therefore significantly easier to solve.
From \shortcite{kalnay20074} we have the standard weak-constaint formultation with superscript denoting the timestep within the assimilation window (say there are $0,\ldots,N$ observation time steps within the assimilation window):
...
We have already discussed a few such related appraches: the use of the Kalman gain matrix $\mbK$ used within the 3D-Var cost function, the ``NCEP'' method for 3D-Var, and inflating the ETKF covariance with the 3D-Var static (climatological) covariance $\mbB$.

\subsection{Ensemble-variational hybrids}

In ensemble-variational hybrid approaches benefits of flow dependent covariance estimates obtained from an ensemble method, and the computational efficiency of iteratively solving a cost function to obtain the analysis, are combined.
