...
%% Nonadiabatic and frictional terms have been ignored in the body of the discussion because it was thought that one should first seek to determine how much of the motion could be explained without them.
...
%% It is with this goal that we continue the march forward.
...
Assimilation is the combination of these observations and the predictive model in such a way that minimizes the error of the future prediction based on this analysis.
...
\begin{figure}[h!]
  ...
  \caption{
    ...
    $S_0$, represented by the yellow arrow, is energy coming from the sun that is absorbed by Earth's surface (horizontal green bar).
    The red, dashed arrow is energy radiating out of Earth's surface, of which some is reflected back by the atmosphere (horizontal gray line), and the remainder escapes.
...
In chapter 3, we present a derivation of Ehrhard-M\"{u}ller Equations governing the flow in the loop following the work of \cite{harris2011predicting}.
...
We find that below a critical threshold of observations, predictions are useless (where useless forecasts are those with greater than 70\% RMSE).
...
And finally, we present a ``foamLab,'' a testbed for DA which links models, DA techniques, and localization in a general framework that can be used to extend these results and test experimental methods for DA within different models.
