...
Data is saved for every half second, and is subsampled for 2,4,8,16 and 32 synthetic temperature sensors spaced evenly around the loop.
...
For less than 1000 observations, (1000 being the number of angular slices) observations are placed at cell centers, spaced maximally apart.
For 1000 observations both slice center (midpoint going all the way around) and staggered (closer to one wall, alternating) are considered.
...
This is accomplished for these results by dividing through by the mean climatological values from the observations and model results before assimilation. 
...
Our results indicate that (initially, not plotted) the averaging of temperature over many cells for the synthetic temperature sensors up to 32 sensors significantly degraded prediction skill.
...
In Figure \ref{fig:covlocaladaptive}, the potential gain from adaptive covariance localization can be seen.
