...
\begin{figure}[h!]
  ...
  \caption[With a fixed choice of solver, boundary conditions, and initial conditions that lead to stable convective state, we present the long-term behavior of the temperature difference $\Delta T_{3-9}$ for different meshes]{
    With a fixed choice of solver, boundary conditions, and initial conditions that lead to stable convective state, we present the long-term behavior of the temperature difference $\Delta T_{3-9}$ for different meshes.
    We choose a fixed time step of 0.006 for each simulation, and run the solver for 30 hours on 8 cores.
    The computational limit of mesh creation was a memory limit at 818280 cells, so we present results for meshes starting at 400 cells and cells decreasing in size by a factor of 1.25 in both $y$ and $z$ up to a mesh of 523584 cells.
  }
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.79\textwidth]{/Users/andyreagan/work/2013/2013-05data-assimilation/src/experiments/openFoamAssimilation/data/synData-tStep.01-b290-t340-m40000-noRenum/errorAll002.pdf}
  \caption[The RMSE of forecasts at the end of 40 assimilation cycles for differing observational density is plotted]{
    ...
    More than 2000 observations, which is 5\% of the model resolution, do not improve forecast skill.
    Since only temperature observations are used, 2000 observations represent less than 1\% of observable variables.
  }
  \label{fig:obsexp}
\end{figure}
